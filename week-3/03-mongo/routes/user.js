...
router.post('/signup', async (req, res) => {
    ...
    try{
        ...
        const userfind = await dbfile.User.findOne({username: entry.username});
        if(userfind) return res.send("User Already Exists");
        else{
            const newUser = new dbfile.User(entry);
            await newUser.save();
...
async function findcourse(_id){
    try{
        const coursefound = await dbfile.Course.findOne({id:_id});
        return coursefound;
    }
    catch{
